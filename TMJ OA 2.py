...
# VGG16 모델 구성 및 미세 조정 (마지막 8개의 레이어 학습 가능)
base_model = VGG16(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
for layer in base_model.layers[:-8]:  # 마지막 8개 레이어만 학습
...
model = Sequential([
    ...
    BatchNormalization(),  # 배치 정규화 위치 조정
    Dropout(0.3),  # 드롭아웃 비율 조정
    Dense(1, activation='sigmoid')  # 이진 분류 출력을 위한 출력층
])

# 모델 컴파일
optimizer = Adam(learning_rate=0.0001)  # 학습률 증가
...
reduce_lr = ReduceLROnPlateau(monitor='val_auc', factor=0.5, patience=3, mode='max', min_lr=1e-6)  # 감축 팩터 및 최소 학습률 조정
