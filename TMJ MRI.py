...
source_dir = 'C:/TMJ OA_MRI/source_data'
train_dir = 'C:/TMJ OA_MRI/train_data'
validation_dir = 'C:/TMJ OA_MRI/validation_data'
test_dir = 'C:/TMJ OA_MRI/test_data'

# Excel 파일 로드 및 OA 상태를 딕셔너리로 저장 (ID별로 분류)
oa_data = pd.read_excel(excel_path)
oa_dict = {row['Patient ID']: (row['MRI_Rt OA (0=normal, 1=OA)'],
                               row['MRI_Lt OA (0=normal, 1=OA)'])
           for _, row in oa_data.iterrows()}

# 이미지 필터링 (우측: 1.JPG, 좌측: 2.JPG 이미지만 선택)
all_files = [f for f in os.listdir(source_dir) if f.endswith('1.JPG') or f.endswith('2.JPG')]
...
lt_oa_files = [f for f in all_files if f.endswith('2.JPG') and oa_dict.get(int(f.split(' ')[0]), (0, 0))[1] == 1]
lt_normal_files = [f for f in all_files if f.endswith('2.JPG') and oa_dict.get(int(f.split(' ')[0]), (0, 0))[1] == 0]

# 파일 개수 출력
print(f"rt_oa_files: {len(rt_oa_files)}")
print(f"rt_normal_files: {len(rt_normal_files)}")
print(f"lt_oa_files: {len(lt_oa_files)}")
print(f"lt_normal_files: {len(lt_normal_files)}")
...
def copy_files(file_list, target_dir, oa_dict):
    for filename in file_list:
        ...
        elif filename.endswith('2.JPG'):  # 좌측
